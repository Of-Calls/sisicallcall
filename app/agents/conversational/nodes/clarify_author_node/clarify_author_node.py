...
[4. 이해 표시 — clarify_count=0 + matched_keywords 있을 때만]
사용자가 말한 핵심 단어를 그대로 인용해 agent 가 무엇을 이해했는지 알리는 형식:
- "[matched 첫 항목]이라고만 이해했어요. 더 자세히 말씀해 주시겠어요?"
- "[matched 첫 항목] 관련해서 어떤 점이 궁금하실까요?"
규칙:
- 인용은 반드시 matched_keywords 의 항목만 사용 — refined_text 의 노이즈
  (필러, 의미 없는 토큰) 는 인용 금지.
- matched_keywords 가 비어있으면 본 섹션 적용 금지 — 일반 open 질문([1] 단계 0)으로 폴백.
- [3. STT 오인식 paraphrase] 와 [4. 이해 표시] 가 동시 트리거되면 [3] 우선.
- clarify_count > 0 (객관식/이분법/단답 단계) 에는 적용하지 않음.
...
입력: refined_text="대중 교통" / clarify_count=0 / matched=["대중교통"] / rag_probe={{"top_distance":1.25}}
→ {{"clarify_question": "대중교통이라고만 이해했어요. 어떤 부분이 궁금하실까요?"}}

입력: refined_text="대중 교통 어떻게" / clarify_count=0 / matched=["대중교통"] / rag_probe={{"top_topic":"오시는 길"}}
→ {{"clarify_question": "대중교통 관련해서 어떤 점이 궁금하실까요?"}}
"""
