...
async def delete_voiceprint(call_id: str) -> Response | JSONResponse:
    """특정 통화(call_id / streamSid)의 medium·finetuned voiceprint 및 enrollment 상태 삭제."""
    removed_m = get_onnx_pipeline_service().cleanup(call_id)
    removed_f = get_finetuned_onnx_service().cleanup(call_id)
    voice_enrollment.cleanup(call_id)
    if not removed_m and not removed_f:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"detail": "해당 call_id voiceprint 없음"},
        )
    _logger.info(
        "voiceprint DELETE call_id=%s medium=%s finetuned=%s",
        call_id,
        removed_m,
        removed_f,
    )
