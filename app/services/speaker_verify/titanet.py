...
class TitaNetSpeakerVerifyService(BaseSpeakerVerifyService):
    def __init__(self, model_name: str | None = None) -> None:
        self._model_name = model_name or settings.titanet_model_name
        self._voiceprints: dict[str, np.ndarray] = {}
        logger.info(f"TitaNet 모델 로드 중... (model={self._model_name}, device={_DEVICE})")
        try:
            ...
            self._model.eval()
            logger.info(f"TitaNet 모델 로드 완료 (model={self._model_name})")
    ...
    def _extract_embedding_sync(self, audio_chunk: bytes) -> np.ndarray:
        ...
        audio_signal_len = torch.tensor([audio_signal.shape[1]]).to(_DEVICE)
        with torch.no_grad():
