...
class OnnxMelSpeakerVerifyService(BaseSpeakerVerifyService):
    ...
    def _cap_mel_time_finetuned_onnx(self, mel: np.ndarray) -> np.ndarray:
        """구 ONNX(내부 T≈600 고정 Where) 대비 mel 프레임 상한. 0 이면 미적용."""
        if self._settings_field != "titanet_finetuned_onnx_path":
            return mel
        mx = int(settings.titanet_finetuned_onnx_max_mel_frames)
        if mx <= 0:
            return mel
        ...
        if tdim <= mx:
            return mel
        logger.info(
            "[%s] mel 시간축 자름 T=%d→%d (TITANET_FINETUNED_ONNX_MAX_MEL_FRAMES)",
            self._log_tag,
            tdim,
            mx,
        )
    ...
    async def extract_and_store(self, audio_chunk: bytes, call_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                "[%s] voiceprint 등록 call_id=%s dim=%d",
                self._log_tag,
                call_id,
                embedding.shape[0],
            )
...
def log_onnx_inference_session_check() -> None:
    ...
    logger.info(
        "finetuned session id: %d", id(ft.session) if ft.session is not None else 0
    )
    logger.info(
        "medium session id:    %d", id(md.session) if md.session is not None else 0
    )
    logger.info("ONNX verifier 객체 id: medium=%d finetuned=%d", id(md), id(ft))
